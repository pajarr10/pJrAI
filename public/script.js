/* ============================================================
   pJrAI — script.js  (with backend tracking)
   ============================================================ */
(function () {
  "use strict";

  /* ── Backend base URL ─────────────────────────────────────────
     If you deploy to Vercel/VPS, change this to your domain.
     Empty string "" means same-origin (recommended for production).
  ──────────────────────────────────────────────────────────────── */
  var BACKEND_URL = "";           // e.g. "https://pjrai-backend.vercel.app"
  var API_URL     = "https://api-nanzz.my.id/docs/api/ai/worm-gpt.php?prompt=halo";
  var STORAGE_KEY = "pjrai_chat_history";

  /* ── Session ID ───────────────────────────────────────────── */
  function getSessionId() {
    var key = "pjrai_session_id";
    var id;
  ...
  function trackVisit() {
    ...
    }).catch(function () {}); // silent fail — never block UI
  }

  /* ── Log chat to backend ──────────────────────────────────── */
  function logChat(userMessage, aiResponse) {
  ...
  function setThemeIcon(theme) {
    if (theme === "dark") {
      themeIcon.innerHTML = '<svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="#000" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"><path d="M21 12.8A9 9 0 1 1 11.2 3 7 7 0 0 0 21 12.8z"/></svg>';
    } else {
      themeIcon.innerHTML = '<svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="#000" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="4"/><path d="M12 2v2M12 20v2M4.9 4.9l1.4 1.4M17.7 17.7l1.4 1.4M2 12h2M20 12h2M4.9 19.1l1.4-1.4M17.7 6.3l1.4-1.4"/></svg>';
    }
  }
  ...
  document.querySelectorAll(".faq-item__q").forEach(function (btn) {
    btn.addEventListener("click", function () {
      ...
      if (expanded) { btn.setAttribute("aria-expanded", "false"); panel.style.maxHeight = null; }
      else { btn.setAttribute("aria-expanded", "true"); panel.style.maxHeight = panel.scrollHeight + "px"; }
    });
  });
  ...
  var activeTyping    = null;
  var isBusy          = false;

  function loadHistory() {
    try { var raw = localStorage.getItem(STORAGE_KEY); if (raw) history = JSON.parse(raw) || []; }
    catch (e) { history = []; }
  }
  ...
  function renderMessage(role, text, time, opts) {
    ...
    if (role === "ai" && !opts.error && !opts.pending) {
      ...
      copyBtn.addEventListener("click", function () {
        ...
        if (navigator.clipboard && navigator.clipboard.writeText) { navigator.clipboard.writeText(doneText); }
  ...
  function renderAll() {
    ...
    history.forEach(function (m) { renderMessage(m.role, m.text, m.time, { error: m.error }); });
    scrollToBottom();
  }

  function renderWelcome() {
    renderMessage("ai", "Halo! Saya pJrAI, asisten AI bertenaga DeepSeek-R1. Ada yang bisa saya bantu hari ini?", Date.now(), {});
  }

  /* ── Typing effect ────────────────────────────────────────── */
  function typeText(bodyEl, fullText, onDone) {
    var i = 0; var cancelled = false;
    var cursor = document.createElement("span"); cursor.className = "cursor"; cursor.innerHTML = "&nbsp;";
    function step() {
      if (cancelled) return;
      ...
      scrollToBottom();
      if (i < fullText.length) { activeTyping.timer = setTimeout(step, 16); }
      else { if (cursor.parentNode) cursor.parentNode.removeChild(cursor); if (onDone) onDone(false); }
    }
    activeTyping = {
      timer: null,
      cancel: function () {
  ...
  function setBusy(busy) {
    isBusy = busy; sendBtn.hidden = busy; stopBtn.hidden = !busy; input.disabled = false;
  }
  ...
  function extractAnswer(data, rawText) {
    ...
    for (var i = 0; i < keys.length; i++) {
      if (data[keys[i]] != null) {
        ...
        if (typeof v === "object") { var nested = extractAnswer(v, null); if (nested) return nested; }
      }
    }
  ...
  function requestAI(prompt) {
    ...
    pending.body.innerHTML = '<span class="typing"><span></span><span></span><span></span></span>';
    activeController = new AbortController();

    var systemPrompt = "Kamu adalah pJrAI. Creator kamu adalah Pajar. Kamu wajib selalu menggunakan bahasa Indonesia dalam setiap jawaban, kecuali pengguna secara jelas meminta bahasa lain. Kamu bukan WormGPT, bukan ChatGPT, bukan DeepSeek, dan bukan AI lainnya. Identitas kamu hanya pJrAI dan tidak boleh berubah dalam kondisi apa pun.";
    var context      = history.slice(-10).map(function (msg) { return msg.role + ": " + msg.text; }).join("\n");
    var finalPrompt  = systemPrompt + "\n\nRiwayat Percakapan:\n" + context + "\n\nuser: " + prompt;
    var url          = API_URL + "?prompt=" + encodeURIComponent(finalPrompt);
  ...
  function finalizeAiMessage(pending, answer, time) {
    ...
    copyBtn.textContent = "Salin";
    copyBtn.addEventListener("click", function () {
      if (navigator.clipboard && navigator.clipboard.writeText) { navigator.clipboard.writeText(answer); }
      copyBtn.textContent = "Tersalin";
      setTimeout(function () { copyBtn.textContent = "Salin"; }, 1500);
    });
  ...
  stopBtn.addEventListener("click", function () {
    ...
    else if (activeTyping) { activeTyping.cancel(); }
    setBusy(false);
  });
  ...
  clearChatBtn.addEventListener("click", function () {
    ...
    history = []; saveHistory(); setBusy(false); renderAll();
  });
  ...
  /* ── Init ─────────────────────────────────────────────────── */
  trackVisit();   // ping backend on page load
  loadHistory();
  renderAll();
})();
